...
export async function getCurrentTrack() {
  return call<SpotifyPlayer>("me/player").then(createTrackInfoData);
}

export async function playOrPause() {
  return getCurrentTrack().then((track) => {
    if (track) {
...
export async function fetchUserName() {
  ...
  return call("me").then((data: SpotifyMe) => data.display_name);
}
...
async function call<T>(
  ...
  return fetch(baseUrl + path, {
    ...
    if (response.status === 401 && retryNo < 1) {
      return fetchTokenByRefreshToken().then(() =>
        call(path, method, retryNo + 1),
      );
    }
